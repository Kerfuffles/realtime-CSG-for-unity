    ...
    public class UnityInternalIcons : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if( GUILayout.Button( "Export", EditorStyles.toolbarButton ) )
            {
                ...
                {
                    Texture2D tex   = EditorGUIUtility.IconContent( _icons[i].icon.image.name ).image as Texture2D;
                    Texture2D final = new Texture2D( tex.width, tex.height, tex.format, false );

                    Graphics.CopyTexture( tex, 0, 0, final, 0, 0 );

                    string path = $"{Directory.GetParent( Application.dataPath.Replace( "/", "\\" ) ).FullName}\\Icons\\{_icons[i].icon.image.name}.png";

                    if( !Directory.Exists( $"{Directory.GetParent( Application.dataPath.Replace( "/", "\\" ) ).FullName}\\Icons\\" ) )
                    {
                        Directory.CreateDirectory( $"{Directory.GetParent( Application.dataPath.Replace( "/", "\\" ) ).FullName}\\Icons\\" );
                    }

                    File.WriteAllBytes( path, final.EncodeToPNG() );
                    Debug.Log( $"Saved image '{_icons[i].icon.image.name}' to '{path}'" );
                }
            }
